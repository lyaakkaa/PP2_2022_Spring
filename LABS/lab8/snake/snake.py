...
# загружаем картинки
background = pygame.image.load('./snake_img/sand.jpg')
background = pygame.transform.scale(background,(WIDTH, HEIGHT- 50)) 
apple = pygame.image.load('./snake_img/apple.png') 
apple_Score = pygame.image.load('snake_img/apple_score.png') 
level_up = pygame.image.load('snake_img/level-up.png')
br = pygame.image.load('./snake_img/brick-wall.png') 
...
while restart: 
    ...
    while not finished: # начинаем наш игровой цикл 
        ...
        walls_coor = open(f'./levels/wall{level}.txt', 'r').readlines()  # открываем файл с уровнем
        walls = []
